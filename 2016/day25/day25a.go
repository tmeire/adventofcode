...
func (i OUTInstruction) preload(cpu *CPU) {
	if v, err := strconv.Atoi(i.o); err == nil {
		cpu.reg[i.o] = v
	}
}

func (i OUTInstruction) apply(cpu *CPU) int {
	cpu.write(cpu.reg[i.o])
	return 1
}

func (i OUTInstruction) toggle() instruction {
	return INCInstruction{i.o}
}
...
func (cpu *CPU) write(i int) {
	cpu.out = append(cpu.out, i)
}
...
func (cpu *CPU) execute() []int {
	cpu.ip = 0
	for i := 0; cpu.ip < len(cpu.program) && i < MAX_INSTRUCTIONS; i++ {
...
func main() {
	...
	i := 0
	for matched := false; !matched && i < 1000; {
		clone := append([]instruction(nil), cmds...)
